...
int row_start;
int row_end;
int my_rows;  // Número real de filas que procesa este rank
...
void set_ui()
{
    ...
    while (window.isOpen())
    {
        ...
        // Buffer temporal para recibir datos (incluye padding)
        uint32_t *recv_buffer = new uint32_t[WIDTH * delta * nprocs];
        
        MPI_Gather(
            pixel_buffer, WIDTH * delta, MPI_UNSIGNED,  // todos envían WIDTH * delta
            recv_buffer, WIDTH * delta, MPI_UNSIGNED,   // rank 0 recibe WIDTH * delta de cada uno
            0, MPI_COMM_WORLD);
        
        // Copiar solo los primeros HEIGHT filas (ignorar las últimas 'padding' filas)
        std::memcpy(
            texture_buffer,               // destino
            recv_buffer,                  // origen  
            WIDTH * HEIGHT * sizeof(uint32_t));  // copiar WIDTH * HEIGHT píxeles
        // Las últimas (padding * WIDTH) píxeles de recv_buffer se ignoran
...
int main(int argc, char **argv)
{
    ...
    padding = delta * nprocs - HEIGHT;  // filas de padding totales
    ...
    my_rows = row_end - row_start;  // Número real de filas de este proceso

    // icicializar buffers
    pixel_buffer = new uint32_t[WIDTH * delta];
    std::memset(pixel_buffer, 0, WIDTH * delta * sizeof(uint32_t));

    fmt::print("Rank {}, nprocs {}, delta {}, row_start {}, row_end {}, my_rows {}\n",
               rank, nprocs, delta, row_start, row_end, my_rows);
    ...
    else
    {
        while (true)
        {
            ...
            MPI_Gather(
                pixel_buffer, WIDTH * delta, MPI_UNSIGNED,  // todos envían WIDTH * delta
                nullptr, 0, MPI_UNSIGNED,                   // workers no reciben nada
                0, MPI_COMM_WORLD);
